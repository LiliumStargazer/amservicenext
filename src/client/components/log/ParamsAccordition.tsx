...
import useQueryGetParam from "@/src/client/hooks/useQueryGetParam";
import { v4 as uuidv4 } from 'uuid';
import { TypeConfigParams, TypeDeviceParams, TypeErogParams, TypeFavoriteParams, TypeFingerParams, TypeHopperParam,
    TypeIngParam, TypeMoneyLineAccceptParam, TypePanelParam,
    TypePaxParams,
    TypeTerminalList,
    TypeVideoWeek,
    TypeVolume, TypeVteContractParam } from "../../types/types";
import AlertLoading from "@/src/client/components/shared/AlertLoading";

const ParamsAccordition: React.FC = () => {
    ...
    const { isLoading, isError, isSuccess, data, error } = useQueryGetParam();
    ...
    const deviceParams: TypeConfigParams[] = param.Device ? param.Device.map((device: any) => ({
    ...
    const ageParam: TypeDeviceParams[] = param.EtaLimite ? param.EtaLimite.map((age: any, index:any) => ({
        label: index?.toString(),
        value: age?.toString(),
    })) : [];

    const mapErogParams = (erogList: any[]) => {
        return erogList.map((erog: any) => ({
    ...
    const favoritesParams: TypeFavoriteParams[] = param.Favorites ? param.Favorites.map((favorite: any) => ({
        key: favorite.ItemTitle?.toString(),
        title: favorite.ItemTitle?.toString(),
        subtitle: favorite.ItemSubtitle?.toString(),
    })) : [];

    const fingerParams: TypeFingerParams[] = param.FingerInfo ? param.FingerInfo.map((finger: any) => ({
        ID: finger.FingerID?.toString(),
        quality: finger.EnrolledQuality?.toString(),
        cf: finger.PeopleID?.toString(),
    })) : [];

    const hopperParam: TypeHopperParam[] = param.HopperParam ? param.HopperParam.map((hopper: any) => ({
        key: hopper.Name?.toString(),
        coinvalue: hopper.CoinValue?.toString(),
        canalesmistamento: hopper.CoinSorter?.toString(),
    })) : [];

    const moneyLineAccceptParam: TypeMoneyLineAccceptParam[] = param.MoneyLineAcceptParam ? param.MoneyLineAcceptParam.map((param: any) => ({
    ...
    const panelParam: TypePanelParam[] = param.TsParam ? param.TsParam.map((panel: any) => ({
    ...
    const vteContractParam: TypeVteContractParam[] = param.VTEContractPackages ? param.VTEContractPackages.map((vte: any) => ({
        name: vte.name?.toString(),
        dataStart : vte.datastart?.toString(),
        dataEnd: vte.dataend?.toString(),
    })) : [];

    const videoWeek: TypeVideoWeek[] = param.VideoWeek ? param.VideoWeek.map((video: any, index: any) => ({
        number: index?.toString(),
        file : video?.toString(),
    })) : [];

    const volume: TypeVolume[] = param.VolumeDays ? param.VolumeDays.map((volume: any) => ({
    ...
    const paxParam: TypePaxParams[] = param.Device ? param.Device.flatMap((device: any) =>
    ...
    const ingParam: TypeIngParam[] = param.Device ? param.Device.flatMap((device: any) =>
    ...
    const mapTerminalList = (terminalList: any[]) => {
        return terminalList.map((terminal: any, index: any) => ({
    ...
    const terminalList: TypeTerminalList[] = param.Device ? param.Device.flatMap((device: any) => {
...
export default ParamsAccordition;
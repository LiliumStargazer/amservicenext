...
import { filterParams, getRowStyle, handleExtractClick, matchLogEventWithAliveEvents } from "@/lib/aggrid-helper";
...
export function useAgGridConfigLogMaster({ rows, getRowIds, isExtracted = false }: UseAgGridConfigLogMasterProps) {
    ...
    const colDefsBase = useMemo(() => [
        ...
        { headerName: 'Tag Data', field: 'TagData', flex: 2, cellStyle: { whiteSpace: 'nowrap' }, filter: false, sortable: false, floatingFilter: false},
    ], []);
        ...
        if (event.colDef.field === 'EventString') {
            const fetchData = async () => {
                try {
                    ...
                    if (matchedEvent) {
                        setDialogContent(String(matchedEvent));
                        setIsDialogOpen(true);
                    } else {

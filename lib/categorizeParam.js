...
function parseTimeStampVideo(fileString) {
    const parsedFiles = [];
    fileString.forEach(fileString => {
        const filenameParts = fileString.split('-');
        const datetimeString = filenameParts.slice(0, 3).join('-');
        const year = datetimeString.slice(0, 4);
        const month = datetimeString.slice(4, 6);
        const day = datetimeString.slice(6, 8);
        const hour = filenameParts[1];
        const minutes = filenameParts[2];
        const seconds = filenameParts[3];
...
const translateSingleValue = (name, value, category) => {
    return {
        category: category,
        name: name,
        value: value,
...
const translateObject = (paramValues, mapping, category) => {
    const values = [];
    ...
    return {category: category, value: values};
};
...
function translateParam(inputObj) {
    ...
    switch (key.toLowerCase()){
        ...
        case "orarioaccensionedeluxe":
            return translateSingleValue("Orario Accensione Deluxe", convertToDateTime(paramValue), translation.category);
        case "orariospegnimentodeluxe":
            return translateSingleValue("Orario Spegnimento Deluxe", convertToDateTime(paramValue), translation.category);
        case "orarioinviovenditore":
            return translateSingleValue("Orario Invio Venditore", convertToDateTime(paramValue), translation.category);
        case "favorites":
        ...
        case "photolevel":
            return translateSingleValue("Fotocellule", photolevel[paramValue].name, "Configurazione");
        case "devicetype":
        ...
        default:{
            ...
            // console.log(paramValue);
            // console.log(translation.category);

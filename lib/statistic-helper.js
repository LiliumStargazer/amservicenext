...
export const filterTechnicianFromTickets = (data) => {
    return data
      .map((ticket) => ticket.Manutentore)
      .filter(
        (Manutentore, index, self) => self.indexOf(Manutentore) === index,
      );
}

export const storedTechnicians = [
    ...
    {Manutentore: "Tagliaferri Luca", Active: true},
...
export const colDefs = [
    {headerName: 'Technician', field: 'Manutentore', flex: 2, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true},
    {headerName: 'Total Tickets', field: 'totalTickets', flex: 1, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true},
    {headerName: 'MTTR days', field: 'MTTR', flex: 1, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true, aggFunc: 'avg'},
    {headerName: '% Tickets with TTR 0-3 days', field: 'percentageOfTicketsWithTimeToRestoreLessOrEqualThree', flex: 1, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true},
    {headerName: '% Tickets with TTR 4-5 days', field: 'percentageOfTicketsWithTimeToRestoreBetweenFourAndFive', flex: 1, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true},
    {headerName: '% Tickets with TTR over 5 days', field: 'percentageOfTicketsWithTimeToRestoreGreaterThanFive', flex: 1, cellStyle: {whiteSpace: 'nowrap'}, filter: false, sortable: true},
];
...
export const filterOnlyActiveTechnicians = (technicians, storedTechnicians) => {
...
export const filterTicketsAndAddTTR = (tickets, activeTechnicians, isOpen) => {
    const typesToExclude = ["No Report", "Telefonico", "TroubleShooting", "--ND--", "Completamento Ordine"];
    const seenNrTickets = new Set();
...
export const createStatistics = (tickets, activeTechnicians) => {
